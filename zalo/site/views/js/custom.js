...
function name(params) {
        ...
        $.get("/zalo/site/controllers/home.php?act=getnumberdata&page=" + nextpage + "&limit=" + limit, function(data, status) {
            ...
            json['data'].forEach(item => {
                ...
                html = '<tr class="div-" id="flip">' +
                    ...
                    '</td>' +
                    '<td class="name hidec">' +
                    '<input type="text" readonly class="number_input btn border-success name" name="name" id="name" value="'+ item['code'] +'">' +
                    '</td>' +
                    '<td class="sortfr hidec">' +
                    '<textarea name="" id="" readonly class=" btn border-success" cols="10"  rows="1">' + item['slbb'] + ' </textarea>' +
                    '</td>' +
                    '<td class="sortnote hidec">' +
                    '<textarea name="note" id="note" class="note btn border-success text-left" cols="30" rows="3">' + item['note'] + '</textarea>' +
                    '</td>' +
                    '<td class="named">' +
                    '<input type="text" readonly class=" btn border-success " name="name" id="name" value="' + item['name'] + '">' +
                    '</td>' +
                    '<td class="avatar">' +
                    '<img  src="' + item['avatar'] + '" class="img-avt" width="50px" />' +
                    '</td>' +

                    '<td class="form-check form-switch hidec  ">' +
                    '<input type="checkbox"  class="used form-check-input ms-2" name="used" ' + checked + ' id="used">' +
                    '</td>' +
                    
                    '<td>' +
                    '<textarea name="cookie" id="cookie" readonly class="cookie btn border-success" cols="30"  rows="3">' + item['cookie'] + ' </textarea>' +
                    '</td>' +
    ...
    $("#form").on("click", ".used", (function(e) {
        ...
        var check = $(tr).children()[7];
        check = $(check).children()[0];

        var note = $(tr).children()[4];
    ...
    $("#form").on("click", ".name", (function(e) {


        var td = $(this).parent();
        var tr = $(td).parent();
        var name = $(tr).children()[2];
    ...
    var big = 0;
    $("#form").on("click", ".img-avt", (function(e) {


        var td = $(this).parent();
        var tr = $(td).parent();
        var td_chi = $(tr).children()[6];
        var img = $(td_chi).children()[0];
        if (big == 0) {
            $(img).css('width', '200px');
            big = 1;
        } else {
...
function myFunction3() {
    ...
    note = document.getElementById("filter");
    keyvalue = note.value.toUpperCase();
    ...
    mydiv0 = document.getElementById("form");
    tr = document.getElementsByClassName("div-");
    // mydiv2 = mydiv1.getElementsById("mydiv2");
    // tr = mydiv1.getElementsByClassName("tr-filter");
       
    var count = 0;  
    for (i = 0; i < tr.length; i++) {
        a = tr[i].getElementsByTagName("textarea")[0];
        a_note = tr[i].getElementsByTagName("textarea")[1];
        a_note= a_note.textContent || a_note.innerText;
        
        txtValue = a.textContent || a.innerText; 
        if ((Number(txtValue) >= Number( from) &&  Number(txtValue) <=Number( to ) )  && a_note.toUpperCase().indexOf(keyvalue)> -1  ) {
            count++; 
            tr[i].style.display = "";
        } else {
